...
start matching from last characters of both strings. If last characters match, then our task reduces 
to n-1 characters. If last characters don't match, then find the position of B's mismatching character in A.
The difference between two positions indicates that these many characters of A must be moved before current
character of A. 
...
public class TransformString {
    ...
    public static int minOperations(String A, String B) {
        int n = A.length();
        int m = B.length();
        // step 1: check if transformation is possible or not 
        // if length of both strings is not same then transformation is not possible
        if(n!=m){
            return -1;
        }
        // if both strings have different set of characters then transformation is not possible
        ...
        // step 2: find minimum number of operations
        int res = 0;
    ...
    public static void main(String[] args) {
        String A = "ABD";
        String B = "BAD";
        System.out.println(minOperations(A, B)); // Output: 1

        A = "EACBD";
        B = "EABCD";
        System.out.println(minOperations(A, B)); // Output: 3
    }
}
